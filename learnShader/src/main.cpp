...
// -----------------------------------
// 渲染循环 交换双缓冲并执行绘制
//void my_renderLoop(GLFWwindow* window, unsigned int& VAO, unsigned int& VBO,unsigned int& EBO, unsigned int& shaderProgram)
void my_renderLoop
(GLFWwindow* window, unsigned int& VAO, unsigned int& EBO,Shader* myShader)
{
...
{
    vertexShader = glCreateShader(GL_VERTEX_SHADER);    //GPU 申请类型
    glShaderSource(vertexShader,1,&vertexShaderSource,nullptr);
    glCompileShader(vertexShader);          //GPU 编译为 binary code

    fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);//GPU 申请类型  
    glShaderSource(fragmentShader,1,&fragmentShaderSource,nullptr);
    glCompileShader(fragmentShader);        //GPU 编译
...
int main()
{
    ...
    Shader* myShader = 
        new Shader(vertexPath,fragmentPath);// 创建并加载着色器
    my_VertexAttribute();         

    my_renderLoop(window,VAO,EBO,myShader);    // 渲染循环
    glfwTerminate();
    return 0;
}
/////////////////////////////////////////////////////////////////////////////////////////////
...
{
    const char* N_file 
        = "assets/vertexShaderSource.txt";
    std::ifstream AAstream;
    AAstream.open(N_file);
    if(!AAstream)
    {
        cout << "ifstream open file"  << " error!" << endl;
    }
    string line;
    while (getline(AAstream,line))
    {
        cout<<"line:"<<line<<endl;
    }
}